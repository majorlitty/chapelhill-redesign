...
export default function PropertyModal({ isOpen, onClose, onBookTour }: PropertyModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-5xl max-h-[90vh] bg-[#f8fafc] rounded-3xl shadow-2xl overflow-hidden flex flex-col border border-white/60">
        
        {/* Header */}
        <div className="p-6 sm:p-8 bg-white border-b border-slate-100 flex items-center justify-between">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-100 text-xs font-semibold uppercase tracking-wider text-slate-700 mb-2">
          ...
          <button
            onClick={onClose}
            className="w-10 h-10 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-600 hover:text-black transition-colors"
              ...
              >
                <div>
                  <div className="relative h-60 w-full overflow-hidden">
                    ...
                    <div className="absolute bottom-3 right-3 bg-white/95 backdrop-blur-md text-[#162521] px-3 py-1 rounded-full text-sm font-bold shadow-xs">
                      {property.price}
                    </div>
                  </div>
                  ...
                  <div className="p-5">
                    ...
                    <h3 className="text-lg font-bold text-[#162521] group-hover:text-[#461313] transition-colors">
                      {property.name}
                    </h3>
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-100 grid grid-cols-3 gap-2 text-xs text-[#162521]/70 font-medium">
                ...
                <div className="p-5 pt-0">
                  <button
                    ...
                    className="w-full bg-[#461313] hover:bg-[#D64933] text-white py-2.5 px-4 rounded-xl font-semibold text-xs sm:text-sm flex items-center justify-center gap-2 transition-colors cursor-pointer shadow-sm"
                  >
        ...
        {/* Footer */}
        <div className="p-4 sm:p-6 bg-slate-50 border-t border-slate-200/80 flex flex-wrap items-center justify-between gap-4 text-xs text-slate-500">
          <p>Chapelhill Private Portfolio • Verified Off-Market and Premier Listings</p>
          <button
            onClick={onClose}
            className="text-slate-700 hover:text-black font-semibold uppercase tracking-wider"
          >
            Back to Overview
          </button>
        </div>
